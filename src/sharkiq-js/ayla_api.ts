...
// New AylaApi object
const get_ayla_api = function(username, password, log, europe = false) {
    if (europe) {
...
class AylaApi {
    ...
    // Make API Request
    makeRequest(method, url, data, auth_header): any {
        var websession = this.websession;
    ...
    // Sign  in
    async sign_in() {
        var login_data = this._login_data;
        var url = `${this.europe ? global_vars.EU_LOGIN_URL : global_vars.LOGIN_URL}/users/sign_in.json`;
        try {
            var resp = await this.makeRequest("POST", url, login_data, null)
            var jsonResponse = JSON.parse(resp.response);
    ...
    // Refresh auth token
    async refresh_auth() {
        var refresh_data = {"user": {"refresh_token": this._refresh_token}}
        var url = `${this.europe ? global_vars.EU_LOGIN_URL : global_vars.LOGIN_URL}/users/refresh_token.json`;
        try {
            var resp = await this.makeRequest("POST", url, refresh_data, null);
            var jsonResponse = JSON.parse(resp.response);
            this._set_credentials(resp.status, jsonResponse);
        } catch {
    ...
    // Sign out
    async sign_out() {
        var url = `${this.europe ? global_vars.EU_LOGIN_URL : global_vars.LOGIN_URL}/users/sign_out.json`;
        try {
    ...
    // List device objects
    async list_devices() {
        var url = `${this.europe ? global_vars.EU_DEVICE_URL : global_vars.DEVICE_URL}/apiv1/devices.json`;
        try {
            var resp = await this.makeRequest("GET", url, null, this.auth_header);
            var devices = JSON.parse(resp.response);
            if(resp.status == 401) {
    ...
    async get_devices(update = true) {
        var europe = this.europe;
        var ayla_api = this;
        try {
            var d = await this.list_devices();
            var devices = d.map(function (device) {
                return new SharkIqVacuum(ayla_api, device, europe = europe);
            });
            if(update) {
